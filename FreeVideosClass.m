...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if(section == 0){
        ...
        if([filteredArrayofConfigObjects count] > 0 && FullSubscription == FALSE ){
            sectionHeader.text = @"GCSE Biology – Start today from only £1.49";
        }
        else if ([filteredArrayofConfigObjects count] > 0 && FullSubscription == TRUE ){
            sectionHeader.text = @"My Courses – GCSE Biology";
        }
        else{
            sectionHeader.text =@"";
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0 ){
        ...
        UIImage* theImage =[ImageObjects objectAtIndex:arc4random() % 75];
...
-(void)ConfigureProductList:(NSString *)ProductID{
    ...
    NSString* OneWeek = @"Biology1week";    //[ProductID stringByAppendingString:@"1week"]; // fixing a naming problem here i made a misake on itunes
